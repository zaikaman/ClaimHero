...
- **AWS Textract OCR & Redacted Clinical Ingestion**: Binary denial documents (PDF/images) are OCR'd exclusively by AWS Textract under HIPAA BAA (fail-hard, zero raw-PHI fallback). OpenAI `gpt-5.4-nano` Structured Outputs process de-identified text only via the mandatory server-side `redactBeforeLLM` gate, extracting CPT codes, ICD-10 codes, CARC/RARC denial reason codes (e.g., CO-50), and disputed dollar amounts with zero raw PHI or binary images transmitted to external models.
