 ...
 *
 * Fail-closed: the standard is forced to HIPAA_SAFE_HARBOR and DOS masking is
 * forced on regardless of caller options, because LLM endpoints are untrusted
 * (no BAA). Payer correspondence is assembled from vaulted claim fields inside
 * the trusted boundary, never from redacted text.
 */
