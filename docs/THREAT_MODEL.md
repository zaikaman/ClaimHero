...
| **THR-09** | **Information Disclosure (PHI to LLMs)** | AST-01 | Protected Health Information leaked to external third-party model APIs via system prompts, user prompts, or embeddings. | **Server-Side Dual-Tier Redaction Gate & AWS Textract Intake (Fail-Hard)**: Mandatory HIPAA Safe Harbor de-identification (`redactBeforeLLM`) enforced on all textual prompt payloads (`systemPrompt`, `userPrompt`), embeddings, and chatbot dialogs in `convex/lib/openai.ts`. Binary document attachments (PDFs/images) are parsed exclusively within the AWS HIPAA BAA boundary via AWS Textract (`convex/lib/textract.ts`), extracting patient identifiers into the private database and sanitizing OCR text prior to LLM dispatch. OpenAI receives zero raw images and zero direct PHI. PDF/image intake without Textract credentials fails hard with an actionable configuration error (`convex/actions/opticalParser.ts`); inbound attachments without OCR are quarantined in storage with a human-review note and never forwarded as raw bytes (`convex/actions/agentMail.ts`). | Enforced |
