...
| **THR-09** | **Information Disclosure (PHI to LLMs)** | AST-01 | Protected Health Information leaked to external third-party model APIs via system prompts, user prompts, or embeddings. | **Server-Side Dual-Tier Redaction Gate**: Mandatory HIPAA Safe Harbor de-identification (`redactBeforeLLM`) enforced on all textual prompt payloads (`systemPrompt`, `userPrompt`), embeddings, and chatbot dialogs in `convex/lib/openai.ts`. Binary document attachments (`fileInputs`, `imageUrls`) processed during intake OCR in `opticalParser.ts` are a documented intake exception requiring an enterprise HIPAA BAA for authentic production records. | Enforced |
