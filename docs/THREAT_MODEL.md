...
| **THR-05** | **Webhook Payload Tampering / Injection** | AST-01, AST-05 | Manipulated webhook JSON modifies message metadata (sender, inbox ID, subject) to hijack existing claims. | **Zero-Trust Message Re-Fetch**: While webhook headers trigger the intake event, background actions (`processInboundIntake`, `processInboundClaimReply`) re-fetch the authentic message and attachments directly from AgentMail's authenticated REST API using `AGENTMAIL_API_KEY`. | Enforced |
