...
| **THR-09** | **Information Disclosure (PHI to LLMs)** | AST-01 | Protected Health Information leaked to external third-party model APIs via system prompts, user prompts, or embeddings. | **Client-Side Local OCR plus Server-Side Redaction Gate & Optional Textract Enhancement**: Mandatory HIPAA Safe Harbor de-identification (`redactBeforeLLM`) enforced on all textual prompt payloads (`systemPrompt`, `userPrompt`), embeddings, and chatbot dialogs in `convex/lib/openai.ts`. Binary document attachments (PDFs/images) are extracted locally in the browser (`src/lib/clientOcr.ts`: pdf.js/tesseract, zero keys, zero PHI egress), with AWS Textract (`convex/lib/textract.ts`) as an optional server-side enhancement under the AWS HIPAA BAA when credentials are configured, extracting patient identifiers into the private database and sanitizing OCR text prior to LLM dispatch. OpenAI receives zero raw images and zero direct PHI. PDF/image intake with neither Textract credentials nor client-extracted text fails hard with an actionable configuration error (`convex/actions/opticalParser.ts`); inbound attachments without server OCR are preserved in storage and flagged `needs_client_ocr` for in-browser extraction with a human-review note, never forwarded as raw bytes (`convex/actions/agentMail.ts`). | Enforced |
