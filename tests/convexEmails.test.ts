...
describe("Convex Emails & Communications API", () => {
  ...
  describe("startInboundIntake, completeInboundIntake & failInboundIntake", () => {
    it("startInboundIntake: returns false if event is already processing or completed", async () => {
      const existing = { _id: "ev1", status: "processing" };
      const mockCtx: any = {
        db: {
          query: vi.fn().mockReturnValue({
            withIndex: vi.fn().mockReturnValue({
      ...
      const res = await (emails.startInboundIntake as any)._handler(mockCtx, {
        eventId: "evt_1",
        messageId: "msg_1",
        inboxId: "in_1",
        sender: "s@test.com",
    ...
    it("startInboundIntake: inserts or patches intake event to processing", async () => {
      const mockCtx: any = {
        db: {
          query: vi.fn().mockReturnValue({
            withIndex: vi.fn().mockReturnValue({
          ...
          insert: vi.fn().mockResolvedValue("evt_row_1"),
        },
      };
      ...
      expect(mockCtx.db.insert).toHaveBeenCalledWith("agentMailIntakeEvents", expect.objectContaining({
        eventId: "evt_1",
        status: "processing",
      }));
    });

    it("completeInboundIntake & failInboundIntake", async () => {
      const existingEv = { _id: "evt_row_1", eventId: "evt_1" };
      const mockCtx: any = {
        db: {
          query: vi.fn().mockReturnValue({
            withIndex: vi.fn().mockReturnValue({
              first: vi.fn().mockResolvedValue(existingEv),
            }),
          }),
          patch: vi.fn().mockResolvedValue(undefined),
        },
      ...
      await (emails.completeInboundIntake as any)._handler(mockCtx, { eventId: "evt_1", claimId: "claim_1" });
      expect(mockCtx.db.patch).toHaveBeenCalledWith("evt_row_1", expect.objectContaining({ status: "completed", claimId: "claim_1" }));

      await (emails.failInboundIntake as any)._handler(mockCtx, { eventId: "evt_1", error: "Fatal parser fault" });
      expect(mockCtx.db.patch).toHaveBeenCalledWith("evt_row_1", expect.objectContaining({ status: "failed", error: "Fatal parser fault" }));
    });
  });
});
