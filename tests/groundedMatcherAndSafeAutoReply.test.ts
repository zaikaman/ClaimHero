...
Appeals Specialist`;
    ...
    it("rehydrates [CLAIM_REF] and dated [DATE] tokens when generated by model", async () => {
      const claimWithFacts = {
        ...baseClaim,
        appealContext: {
          clinicalFacts: {
            imagingAndDiagnostics:
              "Weight-bearing plain radiographs completed on 05/20/2026 confirmed preserved joint spaces.",
          },
        },
      };

      const rawLlmDraft =
        "We acknowledge receipt of your correspondence regarding Claim #[CLAIM_REF]. Specifically the weight-bearing radiographs dated [DATE] satisfy policy criteria under ERISA.";

      vi.spyOn(libOpenAI, "createChatCompletion").mockResolvedValue(rawLlmDraft);

      let qCalls = 0;
      const mockCtx: any = {
        runQuery: vi.fn().mockImplementation(() => {
          qCalls++;
          if (qCalls === 1) return Promise.resolve(claimWithFacts);
          if (qCalls === 2) return Promise.resolve(null);
          return Promise.resolve([]);
        }),
        runMutation: vi.fn().mockResolvedValue(null),
      ...
      expect(res.success).toBe(true);
      expect(res.draftText).toContain("regarding Claim #CLM-REP-200");
      expect(res.draftText).not.toContain("[CLAIM_REF]");
      expect(res.draftText).toContain("weight-bearing radiographs dated 05/20/2026");
      expect(res.draftText).not.toContain("[DATE]");
    });
  });
});
